...
#include "tools/clang_tidy/extra_actions_base.pb.h"
...
std::string GetBuildDirectory() {
  ...
  const char* sandbox_str = "/sandbox/";
  auto sandbox_start = build_dir.find(sandbox_str);
  if (sandbox_start == std::string::npos) return build_dir;
  auto sandbox_end =
      build_dir.find("/execroot", sandbox_start + strlen(sandbox_str));
